...
#' * sigma - MLE estimates for the model standard deviation sigma  
...
ricker_TMB <- function(data,  silent = FALSE, control = TMBcontrol(), 
  ...
  Smax_mean=230000,Smax_sd=230000, rtmb_v=0) {
...
#' deprecated Simple Ricker model estimated with TMB
#'
#' @param data A list or data frame containing Spawners (S) and log(Recruits/Spawners) (logRS) time series. 
#' 
#' @param silent Logical Silent or optimization details? default is FALSE
...
#' rickerTMB(data=harck)
#' 
ricker_TMB_deprecated <- function(data,  silent = FALSE, control = TMBcontrol(), 
  tmb_map = list(), AC=FALSE, priors_flag=1, stan_flag=0,sig_p_sd=1,
  ...
  if(!AC){
    ...
    if(rtmb_v==0){
      tmb_obj <- TMB::MakeADFun(data = tmb_data,
                              ...
                              map = tmb_map,
                              random = tmb_random, 
                              DLL = "Ricker_autocorr_deprecated", 
                              silent = silent)
    }else{
      tmb_obj <- TMB::MakeADFun(data = tmb_data,
                              parameters = tmb_params, 
...
ricker_rw_TMB <- function(data, tv.par=c('a','b','both'), silent = FALSE, 
  ...
  hightlimit <- c(20,20,log(2),log(2),log(2))
    
  nonvariance_fixed_effects<-c("logalpha","logSmax")
  clss <- "Ricker_tv_all"
  #===================================
  # TMB fit
  #===================================
  ...
  structure(list(
    logalpha    = if(tv.par=="b"){tmb_obj$report()$logalpha}else{tmb_obj$report()$logalpha_t},
    beta     = if(tv.par=="a"){tmb_obj$report()$beta}else{tmb_obj$report()$beta_t},
    ...
    sigma_a      = ifelse(tv.par=="a"|tv.par=="both",
                       tmb_obj$report()$siga,
                       NA),
    sigma_b      = ifelse(tv.par=="b"|tv.par=="both",
                       tmb_obj$report()$sigb,
                       NA),
    model      = tmb_opt,
    tmb_data   = tmb_data,
...
#' 
#' 
ricker_rw_TMB_logb <- function(data, tv.par=c('a','b','both'), silent = FALSE, 
  control = TMBcontrol(), ini_param=NULL, tmb_map = list(), priors_flag=1, 
...
ricker_hmm_TMB2_logb <- function(data, 
                           tv.par = c('a','b','both'), 
                           k_regime = 2, 
  ...
  if(tv.par == "a"){
    ...
    tmb_map$vec_logitbeta<-as.factor(rep(NA,k_regime))
    ...
    tmb_map$scalar_logitbeta<-as.factor(NA)
  ...
  tmb_obj <- TMB::MakeADFun(
      ...
      DLL = "SR_HMM_all_logb", silent = silent)
