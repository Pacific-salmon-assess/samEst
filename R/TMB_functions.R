...
#' @param Smax_mean mean for lognormal prior on Smax, default is 230000. Note that transformation to log space is done internally with log_prior_params()
#' @param Smax_sd sd for lognormal prior on Smax, default is 230000. Note that transformation to log space is done internally with log_prior_params()
#' 
#' @details Priors: Weakly informative priors are included for the main parameters of the model:
#' logalpha ~ N(1.5,2.5)
#' logsmax ~ logN(Smax_mean,Smax_sd)
#' sigobs ~ halfN(0,sig_p_sd)  
#' 
#' 
...
ricker_rw_TMB <- function(data, tv.par=c('a','b','both'), silent = FALSE, 
  ...
  structure(list(
    ...
    EDF= ifelse(AICc_type== "conditional",
                       condnpar,
                       NA),
    grad_i= if(AICc_type== "conditional"&useEDF==TRUE){myEDFo$grad_i}else{NA},
    AICc       = AICc,
    BIC        = BIC,
